...
public class BinaryTree<T> {
    ...
    public static <T> int nodes(BinaryTree<T> t) {
        if(t==null) return 0;
            ...
            return nodes(t.right)+nodes(t.left)+1;
    ...
    public static <T> boolean heightBalanced(BinaryTree<T> t) {
        ...
        if(Math.abs(height(t.left)-height(t.right))>1) return false;
        else return (heightBalanced(t.left) && heightBalanced(t.right));
    ...
    public static <T> void levelOrder(BinaryTree<T> root) {
        ArrayList<BinaryTree<T>> myQ = new ArrayList<BinaryTree<T>>();
        if (root == null)
            return;
        myQ.clear();
        myQ.add(root);
        while(!myQ.isEmpty()) {
            BinaryTree<T> t = myQ.remove(0);
            System.out.print(t.data + "\t");
            if(t.left != null)  myQ.add(t.left);
            if(t.right != null) myQ.add(t.right);
        }
