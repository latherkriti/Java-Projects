...
public class BinarySearchTree<T extends Comparable<T>>{
    ...
    public BinaryTree<T> recursiveSearch(BinaryTree<T> t, T key) {
        ...
        if(key.compareTo(t.getData())<0) {
            ...
            if(t.getLeft()!=null) return recursiveSearch(t.getLeft(),key);

        }
        else if(key.compareTo(t.getData())>0) {
            System.out.println(key);
            System.out.println(t.getData());
            if(t.getRight()!=null) return recursiveSearch(t.getRight(),key);

        }
        if(key.compareTo(t.getData())==0) {
            System.out.println("In equal status");
            System.out.println(key);
            System.out.println(t.getData());
//    		BinaryTree<T> bt = new BinaryTree<>();
//    		bt.setData(t.getData());
//    		bt.setParent(t.getParent());
//    		bt.setLeft(t.getRight());
//    		bt.setRight(t.getRight());
            return t;
        }
        else { System.out.println("Could not find the tree node with value" + "\t"+key);
            return null;
        }
